...
use rsix::fs::{makedev, Statx};
use rsix::fs::{RawMode, Stat};
...
impl MetadataExt {
    ...
    pub(crate) fn from(_file: &fs::File, std: &fs::Metadata) -> io::Result<Self> {
        // On rsix-style platforms, the `Metadata` has everything we need.
    ...
    #[inline]
    pub(crate) fn from_rsix(stat: Stat) -> Metadata {
        Metadata {
            ...
            modified: system_time_from_rsix(
                stat.st_mtime.try_into().unwrap(),
                stat.st_mtime_nsec as _,
            ),
            #[cfg(not(target_os = "netbsd"))]
            accessed: system_time_from_rsix(
    ...
    pub(crate) fn from_rsix_statx(statx: Statx) -> Metadata {
        Metadata {
            ...
            modified: system_time_from_rsix(statx.stx_mtime.tv_sec, statx.stx_mtime.tv_nsec as _),
            accessed: system_time_from_rsix(statx.stx_atime.tv_sec, statx.stx_atime.tv_nsec as _),
            created: system_time_from_rsix(statx.stx_btime.tv_sec, statx.stx_btime.tv_nsec as _),
...
fn system_time_from_rsix(sec: i64, nsec: u64) -> Option<SystemTime> {
    SystemClock::UNIX_EPOCH.checked_add(Duration::new(u64::try_from(sec).unwrap(), nsec as _))
}

impl rsix::fs::MetadataExt for MetadataExt {
    #[inline]
    fn dev(&self) -> u64 {
        self.dev
    }
