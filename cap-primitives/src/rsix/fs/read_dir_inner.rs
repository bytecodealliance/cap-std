...
use std::{fmt, fs, io};
use unsafe_io::os::rsix::{AsRawFd, FromRawFd, RawFd};
...
impl ReadDirInner {
    ...
    pub(crate) fn new_unchecked(start: &fs::File, path: &Path) -> io::Result<Self> {
        ...
        Ok(Self {
            raw_fd: dir.as_fd().as_raw_fd(),
            rsix: Arc::new(Mutex::new(Dir::from(dir)?)),
        })
    }
...
impl Iterator for ReadDirInner {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            let entry = match self.rsix.lock().unwrap().read()? {
                Ok(entry) => entry,
