...
pub(super) fn file_metadata(file: &fs::File) -> io::Result<Metadata> {
    ...
    if !FSTAT_PATH_BADF.load(Relaxed) {
        match Metadata::from_file(file) {
            ...
            Err(err) => match rsix::io::Error::from_io_error(&err) {
                // Before Linux 3.6, `fstat` with `O_PATH` returned `EBADF`.
                Some(rsix::io::Error::BADF) => FSTAT_PATH_BADF.store(true, Relaxed),
                _ => return Err(err),
            },
        }
    }
    ...
    Ok(statat(file, "", AtFlags::EMPTY_PATH).map(MetadataExt::from_rsix)?)
}
