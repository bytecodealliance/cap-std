...
/// We use `Arc<std::fs::File>` rather than `tokio::fs::File` because:
/// - `Dir` only needs the file descriptor, not async read/write
/// - All actual filesystem operations go through sync `cap_primitives`
///   calls wrapped in `spawn_blocking`
/// - `tokio::fs::File` doesn't implement `Clone`, so we use `Arc` to
///   enable cloning for use in `spawn_blocking` closures
#[derive(Clone)]
pub struct Dir {
    std_file: std::sync::Arc<fs::File>,
}
...
impl Dir {
    ...
    pub fn from_std_file(std_file: fs::File) -> Self {
        Self {
            std_file: std::sync::Arc::new(std_file),
        }
    }

    /// Consumes `self` and returns an `std::fs::File`, if this is the
    /// sole reference. Returns `Err(self)` if other clones exist.
    #[inline]
    pub fn into_std_file(self) -> Result<fs::File, Self> {
        match std::sync::Arc::try_unwrap(self.std_file) {
            Ok(f) => Ok(f),
            Err(arc) => Err(Self { std_file: arc }),
        }
    }
    ...
    pub async fn open_dir<P: AsRef<Path>>(&self, path: P) -> io::Result<Self> {
        ...
        .await??;
        Ok(Self::from_std_file(dir.into()))
    }
    ...
    pub async fn remove_open_dir(self) -> io::Result<()> {
        let file = std::sync::Arc::try_unwrap(self.std_file).map_err(|_| {
            io::Error::new(
                io::ErrorKind::Other,
                "cannot remove open dir: other references exist",
            )
        })?;
    ...
    ///
    /// This corresponds to [`tokio::fs::symlink`] (on Unix).
    #[cfg(not(windows))]
    #[inline]
    ...
    /// to the directory that contains it.
    ///
    /// This corresponds to [`tokio::fs::symlink_file`] (on Windows).
    #[cfg(windows)]
    #[inline]
    ...
    /// to the directory that contains it.
    ///
    /// This corresponds to [`tokio::fs::symlink_dir`] (on Windows).
    #[cfg(windows)]
    #[inline]
    ...
    ///
    /// This corresponds to [`tokio::net::UnixListener::bind`],
    ...
    ///
    /// This corresponds to [`tokio::net::UnixStream::connect`],
    /// but only accesses paths relative to `self`.
    ///
    /// XXX: This function is not yet implemented.
    #[doc(alias = "connect")]
    ...
    ///
    /// This corresponds to [`tokio::net::UnixDatagram::bind`],
    /// but only accesses paths relative to `self`.
    ///
    /// XXX: This function is not yet implemented.
    #[doc(alias = "bind")]
    ...
    ///
    /// This corresponds to
    /// [`tokio::net::UnixDatagram::connect`], but only
    /// accesses paths relative to `self`.
    ///
    /// XXX: This function is not yet implemented.
    #[doc(alias = "connect")]
    ...
    ///
    /// This corresponds to
    /// [`tokio::net::UnixDatagram::send_to`], but only
    /// accesses paths relative to `self`.
    ///
    /// XXX: This function is not yet implemented.
    #[doc(alias = "send_to")]
    ...
    ///
    /// This is similar to [`std::path::Path::is_dir`] in that it checks
...
impl TryFrom<Dir> for OwnedFd {
    ...
    fn try_from(dir: Dir) -> io::Result<OwnedFd> {
        match std::sync::Arc::try_unwrap(dir.std_file) {
            Ok(f) => Ok(f.into()),
            Err(arc) => Ok(arc.as_fd().try_clone_to_owned()?),
        }
    }
}
...
impl TryFrom<Dir> for OwnedHandle {
    ...
    fn try_from(dir: Dir) -> io::Result<OwnedHandle> {
        match std::sync::Arc::try_unwrap(dir.std_file) {
            Ok(f) => Ok(f.into()),
            Err(arc) => Ok(arc.as_handle().try_clone_to_owned()?),
        }
    }
}
