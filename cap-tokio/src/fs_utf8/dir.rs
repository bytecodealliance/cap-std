...
///
/// This does not directly correspond to anything in `tokio`, however its
/// methods correspond to the [functions in `tokio::fs`] and the
/// constructor methods for [`tokio::fs::File`].
///
/// Unlike `tokio::fs`, this API's `canonicalize` returns a relative path
/// since absolute paths don't interoperate well with the capability model.
///
/// [functions in `tokio::fs`]: https://docs.rs/tokio/latest/tokio/fs/index.html#functions
#[derive(Clone)]
pub struct Dir {
...
impl Dir {
    ...
    ///
    /// This corresponds to [`tokio::fs::File::open`], but only accesses
    ...
    ///
    /// This corresponds to [`tokio::fs::OpenOptions::open`].
    ///
    ...
    ///
    /// This corresponds to [`tokio::fs::create_dir`], but only accesses
    /// paths relative to `self`.
    ///
    ...
    /// are missing.
    ///
    /// This corresponds to [`tokio::fs::create_dir_all`], but only
    ...
    /// builder.
    ///
    /// This corresponds to [`tokio::fs::DirBuilder::create`].
    ///
    /// TODO: async: fix this when we fix https://github.com/bytecodealliance/cap-std/issues/51
    #[inline]
    ...
    ///
    /// This corresponds to [`tokio::fs::File::create`], but only accesses
    /// paths relative to `self`.
    #[inline]
    ...
    ///
    /// This corresponds to [`tokio::fs::canonicalize`], but instead of
    ...
    /// file.
    ///
    /// This corresponds to [`tokio::fs::copy`], but only accesses paths
    /// relative to `self`.
    #[inline]
    ...
    ///
    /// This corresponds to [`tokio::fs::hard_link`], but only accesses
    /// paths relative to `self`.
    #[inline]
    ...
    /// directory, etc.
    ///
    /// This corresponds to [`tokio::fs::metadata`], but only accesses
    /// paths relative to `self`.
    #[inline]
    ...
    ///
    /// This corresponds to [`tokio::fs::read_dir`], but only accesses
    /// paths relative to `self`.
    #[inline]
    ...
    ///
    /// This corresponds to [`tokio::fs::read`], but only accesses paths
    /// relative to `self`.
    #[inline]
    ...
    ///
    /// This corresponds to [`tokio::fs::read_link`], but only accesses
    /// paths relative to `self`.
    #[inline]
    ...
    ///
    /// This corresponds to [`tokio::fs::read_to_string`], but only
    /// accesses paths relative to `self`.
    #[inline]
    ...
    ///
    /// This corresponds to [`tokio::fs::remove_dir`], but only accesses
    /// paths relative to `self`.
    #[inline]
    ...
    /// carefully!
    ///
    /// This corresponds to [`tokio::fs::remove_dir_all`], but only
    /// accesses paths relative to `self`.
    #[inline]
    ...
    ///
    /// This corresponds to [`tokio::fs::remove_file`], but only accesses
    /// paths relative to `self`.
    #[inline]
    ...
    ///
    /// This corresponds to [`tokio::fs::rename`], but only accesses paths
    /// relative to `self`.
    #[inline]
    ...
    ///
    /// This corresponds to [`tokio::fs::set_permissions`], but only
    ...
    ///
    /// This corresponds to [`tokio::fs::symlink_metadata`], but only
    /// accesses paths relative to `self`.
    #[inline]
    ...
    ///
    /// This corresponds to [`tokio::fs::write`], but only accesses paths
    /// relative to `self`.
    #[inline]
    ...
    ///
    /// This corresponds to [`tokio::os::unix::fs::symlink`], but only
    /// accesses paths relative to `self`.
    ///
    /// [`tokio::os::unix::fs::symlink`]: https://docs.rs/tokio/latest/tokio/os/unix/fs/fn.symlink.html
    #[cfg(not(windows))]
    #[inline]
    ...
    /// to the directory that contains it.
    ///
    /// This corresponds to [`tokio::os::windows::fs::symlink_file`], but
    /// only accesses paths relative to `self`.
    ///
    /// [`tokio::os::windows::fs::symlink_file`]: https://docs.rs/tokio/latest/tokio/os/windows/fs/fn.symlink_file.html
    #[cfg(windows)]
    #[inline]
    ...
    /// to the directory that contains it.
    ///
    /// This corresponds to [`tokio::os::windows::fs::symlink_dir`], but
    /// only accesses paths relative to `self`.
    ///
    /// [`tokio::os::windows::fs::symlink_dir`]: https://docs.rs/tokio/latest/tokio/os/windows/fs/fn.symlink_dir.html
    #[cfg(windows)]
    #[inline]
    ...
    ///
    /// This corresponds to [`tokio::os::unix::net::UnixListener::bind`],
    /// but only accesses paths relative to `self`.
    ///
    /// XXX: This function is not yet implemented.
    ///
    /// [`tokio::os::unix::net::UnixListener::bind`]: https://docs.rs/tokio/latest/tokio/os/unix/net/struct.UnixListener.html#method.bind
    #[doc(alias = "bind")]
    #[cfg(unix)]
    #[inline]
    ...
    ///
    /// This corresponds to [`tokio::os::unix::net::UnixStream::connect`],
    /// but only accesses paths relative to `self`.
    ///
    /// XXX: This function is not yet implemented.
    ///
    /// [`tokio::os::unix::net::UnixStream::connect`]: https://docs.rs/tokio/latest/tokio/os/unix/net/struct.UnixStream.html#method.connect
    #[doc(alias = "connect")]
    #[cfg(unix)]
    #[inline]
    ...
    ///
    /// This corresponds to [`tokio::os::unix::net::UnixDatagram::bind`],
    /// but only accesses paths relative to `self`.
    ///
    /// XXX: This function is not yet implemented.
    ///
    /// [`tokio::os::unix::net::UnixDatagram::bind`]: https://docs.rs/tokio/latest/tokio/os/unix/net/struct.UnixDatagram.html#method.bind
    #[doc(alias = "bind")]
    #[cfg(unix)]
    #[inline]
    ...
    ///
    /// This corresponds to
    /// [`tokio::os::unix::net::UnixDatagram::connect`], but only
    /// accesses paths relative to `self`.
    ///
    /// XXX: This function is not yet implemented.
    ///
    /// [`tokio::os::unix::net::UnixDatagram::connect`]: https://docs.rs/tokio/latest/tokio/os/unix/net/struct.UnixDatagram.html#method.connect
    #[doc(alias = "connect")]
    #[cfg(unix)]
    #[inline]
    ...
    ///
    /// This corresponds to
    /// [`tokio::os::unix::net::UnixDatagram::send_to`], but only
    /// accesses paths relative to `self`.
    ///
    /// XXX: This function is not yet implemented.
    ///
    /// [`tokio::os::unix::net::UnixDatagram::send_to`]: https://docs.rs/tokio/latest/tokio/os/unix/net/struct.UnixDatagram.html#method.send_to
    #[doc(alias = "send_to")]
    #[cfg(unix)]
    #[inline]
    ...
    // tokio doesn't have `try_clone`.

    /// Returns `true` if the path points at an existing entity.
    ///
    /// This corresponds to [`tokio::path::Path::exists`], but only
    /// accesses paths relative to `self`.
    #[inline]
    ...
    /// file.
    ///
    /// This corresponds to [`tokio::path::Path::is_file`], but only
    /// accesses paths relative to `self`.
    #[inline]
    ...
    ///
    /// This is similar to [`tokio::path::Path::is_dir`] in that it checks
...
#[cfg(not(windows))]
impl TryFrom<Dir> for OwnedFd {
    type Error = io::Error;
    #[inline]
    fn try_from(dir: Dir) -> io::Result<OwnedFd> {
        let raw = TryIntoRawFd::try_into_raw_fd(dir)?;
...
#[cfg(windows)]
impl TryFrom<Dir> for OwnedHandle {
    type Error = io::Error;
    #[inline]
    fn try_from(dir: Dir) -> io::Result<OwnedHandle> {
        use std::os::windows::io::IntoRawHandle;
        let raw = TryIntoRawHandle::try_into_raw_handle(dir)?;
        Ok(unsafe { std::os::windows::io::OwnedHandle::from_raw_handle(raw) })
    }
}

#[cfg(windows)]
impl TryIntoRawHandleOrSocket for Dir {
...
#[cfg(windows)]
impl TryFrom<Dir> for io_extras::os::windows::OwnedHandleOrSocket {
    type Error = io::Error;
